...
####  1. 사용자가 입력
- 사용자는 입력 장치(Input Device) (예: 키보드, 마우스, 스캐너 등)를 통해 데이터를 시스템에 입력합니다.
- 이 입력 신호는 주소 버스를 통해 기억장치로 전달됩니다.
#### 2. 입력 데이터의 저장
- 입력된 데이터는 일시적으로 메인 메모리(Main Memory, RAM) 에 저장됩니다.
- RAM은 CPU가 직접 접근할 수 있는 휘발성 기억장치로, 연산 중간 단계의 데이터를 보관합니다.
#### 3. 명령어 실행 흐름 시작
- 프로그램 카운터(PC) 는 다음에 실행할 명령어의 주소를 담고 있습니다.
- 이 주소를 기준으로 명령어 레지스터(IR) 가 해당 명령어를 메모리에서 가져옵니다.
#### 4. 명령어 해석 및 제어
- 명령어 레지스터(IR) 는 가져온 명령어를 CPU 내부로 전달합니다.
- 제어장치(Control Unit) 가 명령어를 해석하고, 어떤 연산을 할지, 어떤 데이터를 참조할지 등을 결정합니다.
#### 5. 필요한 데이터 로딩
- 필요한 데이터가 메인 메모리에 없으면, 보조기억장치(Secondary Storage, SSD/HDD) 에 접근하여 데이터를 가져옵니다.
- 이때는 일반적으로 운영체제의 파일 시스템과 DMA가 관여합니다.
#### 6. 속도 향상을 위한 캐시 사용
- 자주 사용되는 데이터는 캐시(Cache) 에 저장됩니다.
- 캐시는 CPU와 가까운 고속 메모리로, 메모리 접근 속도를 획기적으로 줄여줍니다.
#### 7. 산술 및 논리 연산
- 연산 명령이 필요한 경우, 연산장치(ALU) 가 명령어를 수행합니다.
- ALU는 제어장치의 명령에 따라 더하기, 비교, 논리연산 등을 수행합니다.
#### 8. 연산 결과 저장
- 연산 결과는 레지스터(Register) 또는 다시 메인 메모리에 저장됩니다.
- 레지스터는 CPU 내부에 있는 고속 저장 공간으로, 다음 연산에 바로 활용됩니다.
#### 9. 결과 출력
- 최종 결과는 출력 장치(Output Device) (예: 모니터, 프린터 등)를 통해 사용자에게 전달됩니다.
- 또한 주변장치(Peripheral Device) 와 연결되어 추가 기능을 처리할 수 있습니다.
#### 10. 버스를 통한 구성 요소 간 통신
- 모든 구성 요소들은 버스(Bus) 시스템을 통해 연결되어 통신합니다.
  - 주소 버스: 데이터의 위치를 지정
  - 데이터 버스: 데이터를 주고받음
  - 제어 버스: 데이터 흐름과 작업을 제어
#### 11. 주변장치 연동
- Peripheral Device(예: 사운드 카드, 네트워크 인터페이스 등)는 출력장치 또는 다른 구성 요소와 연결되어 특수한 기능을 수행합니다.
